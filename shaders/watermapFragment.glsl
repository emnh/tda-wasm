...
vec3 exchange(float ground, float height, vec2 c, vec2 offset) {
	...
	return vec3(value, offset * value);
}
...
void main() {
	...
	float newHeight = height + diff.x;

	// Rain
	newHeight += 0.000002;
	//if (distance(v_uv, vec2(0.5)) < 0.01) {
	if (distance(v_uv, vec2(0.5) + 0.25 * vec2(cos(u_time), sin(u_time))) < 0.01) {
		newHeight += 0.1;
	}
  ...
  gl_FragColor = vec4(vec3(newHeight, water.yz * 0.5 + diff.yz), 1.0);
