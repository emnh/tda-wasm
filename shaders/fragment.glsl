...
vec3 getDiffuse(vec3 normal, bool isWater) {
  ...
  for (int i = 0; i < maxi; i++) {
    ...
    int ic = i + 1;
    float r = rand(vec2(float(ic), -float(ic)));
    float g = rand(vec2(13.423 * float(ic), -20.562 * float(ic)));
    float b = rand(vec2(17.8925 * float(ic), -4.2593 * float(ic)));
    vec3 color = vec3(r, g, b);
    color.g /= 2.0;
    color = normalize(color);
    float sq = ceil(sqrt(float(maxi) - 0.5));
    float iy = floor((float(i) + 0.5) / sq);
    float ix = float(i) - sq * iy;
    float lx = (ix + 0.5) / sq - 0.5;
    float ly = (iy + 0.5) / sq - 0.5;
